...
// MSL cutscores - these map raw scores to 300-point scale
// Less Than Expected Upper Limit → 100 points
// Expected Score → 150 points  
// Higher Than Expected Threshold → 201 points
 ...
 * @param {number} maxScore - Maximum possible score for this measure
 * @param {number} expectedScore - Score that translates to 150 points (middle of Expected)
 * @param {number} higherThreshold - Score that translates to 201 points (start of Higher range)
 * @param {number} lessUpperLimit - Score that translates to 100 points (top of Less range)
 * @returns {number} Score on 300-point scale
 */
function mapScoreTo300Scale(actualScore, maxScore, expectedScore, higherThreshold, lessUpperLimit) {
  ...
  // lessUpperLimit -> 100 points
  // expectedScore -> 150 points
  // higherThreshold -> 201 points
...
function createMSLRow(index, isIPR = false) {
  ...
  // IPR Cutscores:
  //   - Less Than Expected: < 50
  //   - Expected: 50-89 (middle = 70)
  //   - Higher Than Expected: ≥ 90
  // These map to the 300-point scale as follows:
  //   - Scores 0-49 map to 0-100 points (Less Than Expected)
  ...
  const iprHigher = isIPR ? '90' : ''; // Start of "Highly Effective" (Higher Than Expected)
  const iprReadonly = isIPR ? ' readonly' : '';
  ...
  row.innerHTML = `
    ...
    <div class="msl-section msl-identification">
      <div class="form-group">
        ...
        <input type="text" id="msl-name-${index}" class="msl-name${iprReadonly ? ' readonly-field' : ''}" 
               placeholder="e.g., STAR Reading" value="${iprName}"${iprReadonly}>
      </div>
    </div>
    ...
    <div class="msl-section msl-cutscores">
      ...
      <div class="msl-cutscores-grid">
        <div class="form-group">
          <label for="msl-max-score-${index}">Maximum Score</label>
          <input type="number" id="msl-max-score-${index}" class="msl-max-score${iprReadonly ? ' readonly-field' : ''}" 
                 min="0" step="1" placeholder="e.g., 600" value="${iprMax}"${iprReadonly}>
        </div>
        <div class="form-group">
          <label for="msl-less-upper-limit-${index}">
            <span class="range-indicator less-range">●</span> Less Than Expected (≤)
          </label>
          <input type="number" id="msl-less-upper-limit-${index}" class="msl-less-upper-limit${iprReadonly ? ' readonly-field' : ''}" 
                 min="0" step="0.1" placeholder="Top of Less range" value="${iprLessUpper}"${iprReadonly}>
        </div>
        <div class="form-group">
          <label for="msl-expected-score-${index}">
            <span class="range-indicator expected-range">●</span> Expected (middle)
          </label>
          <input type="number" id="msl-expected-score-${index}" class="msl-expected-score${iprReadonly ? ' readonly-field' : ''}" 
                 min="0" step="0.1" placeholder="Middle of Expected" value="${iprExpected}"${iprReadonly}>
        </div>
        <div class="form-group">
          <label for="msl-higher-threshold-${index}">
            <span class="range-indicator higher-range">●</span> Higher Than Expected (≥)
          </label>
          <input type="number" id="msl-higher-threshold-${index}" class="msl-higher-threshold${iprReadonly ? ' readonly-field' : ''}" 
                 min="0" step="0.1" placeholder="Start of Higher range" value="${iprHigher}"${iprReadonly}>
        </div>
      </div>
    </div>
    ...
    <div class="msl-section msl-actual-section">
      <div class="form-group">
        <label for="msl-actual-score-${index}">Actual Score Achieved</label>
        <input type="number" id="msl-actual-score-${index}" class="msl-actual-score" 
               min="0" step="0.1" placeholder="Score earned">
      </div>
    </div>
  `;
  
  // Attach listeners with debouncing and negative prevention to all number inputs
  ...
  // Attach listener to name input
  row.querySelector('.msl-name')?.addEventListener('input', debouncedUpdate);
...
function resetAll() {
  ...
  // Reset MSL to 2 rows (IPR + one blank)
  const container = document.getElementById('msl-list');
  if (container) {
    container.innerHTML = '';
    mslRowCounter = 0;
    // First row is always IPR
    mslRowCounter++;
    const iprRow = createMSLRow(mslRowCounter, true);
...
function generateCalculationBreakdown() {
  ...
  html += '<p style="font-size: 0.9rem; color: var(--color-muted); margin-bottom: var(--spacing-md);">Using cutscore-based precise scoring</p>';
  
  mslResult.measures.forEach((measure, idx) => {
    html += `<div class="calc-step"><strong>Measure ${idx + 1}: ${measure.name}</strong></div>`;
    html += `<div class="calc-formula">Actual Score: ${measure.actualScore} / ${measure.maxScore} = ${measure.percentage.toFixed(2)}%</div>`;
    html += `<div class="calc-formula">Cutscores: Less≤${measure.lessUpperLimit} | Expected=${measure.expectedScore} | Higher≥${measure.higherThreshold}</div>`;
    html += `<div class="calc-formula">Rating Score (300-scale): ${measure.unweightedScore300} points</div>`;
    html += `<div class="calc-formula">Weighted Score: ${measure.percentage.toFixed(2)}% × ${measure.weight}% × 10 = ${measure.weightedScore} points</div>`;
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize MSL rows - first row is always IPR
  const container = document.getElementById('msl-list');
  if (container) {
    // Row 1: IPR (pre-filled)
    const iprRow = createMSLRow(1, true);
    iprRow.classList.add('visible');
    container.appendChild(iprRow);
    // Row 2: Blank
    const row2 = createMSLRow(2, false);
    row2.classList.add('visible');
    container.appendChild(row2);
  }
...
function loadSampleData() {
  ...
  // Set up 2 MSL measures with sample data (preserve IPR)
  const container = document.getElementById('msl-list');
  if (container) {
    // Clear existing
    container.innerHTML = '';
    ...
    setTimeout(() => {
      ...
      const name2 = document.getElementById('msl-name-2');
      const max2 = document.getElementById('msl-max-score-2');
      const less2 = document.getElementById('msl-less-upper-limit-2');
      const expected2 = document.getElementById('msl-expected-score-2');
      ...
      if (expected2) expected2.value = '350';
      if (higher2) higher2.value = '450';
      if (actual2) actual2.value = '400';
